...
ADD CONSTRAINT `strategy_event_failed_strategy_id_fk` FOREIGN KEY (`strategy_id`) REFERENCES `strategy` (`id`) ON
DELETE
CASCADE ON
UPDATE CASCADE;
...
-- 插件增加备注
ALTER TABLE `host_group_plugin`
    ADD COLUMN `comment` varchar(1024) NOT NULL DEFAULT '' AFTER `timeout`;
ALTER TABLE `host_plugin`
    ADD COLUMN `comment` varchar(1024) NOT NULL DEFAULT '' AFTER `timeout`;
ALTER TABLE `plugin`
    ADD COLUMN `comment` text NOT NULL AFTER `creator`;
SET
